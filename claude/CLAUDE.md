...
- **Serena**：基于 LSP 的项目级智能记忆，提供符号级代码分析与编辑能力
  - 核心能力：语义理解、符号定位与编辑、多语言支持（Python/JS/Java 等）
  - 记忆内容：自动捕获架构约束、模块陷阱、技术决策
  - 适用场景：发现重要 bug 根因、识别反模式、记录技术限制
  - 命名规范：`pitfalls_<module>` / `constraints_<topic>` / `architecture_<component>`
- **Basic Memory**：本地优先的手动开发日志（Markdown 格式，集成 Obsidian）
  - 核心能力：知识图谱导航（memory:// URLs）、本地存储、完全控制
  - 适用场景：阶段性总结、跨会话待办事项、长期参考信息
  - 触发时机：开发阶段完成、收工前、需要记录重要上下文时
  - 组织方式：按日期或功能模块分类（由用户决定）
- **使用原则**：
  - Serena 偏向"自动化、结构化、可检索"的技术知识（LSP 支持的符号级操作）
  - Basic Memory 偏向"叙事性、人工整理"的进展日志（本地 Markdown 文件）
  - 两者互补：Serena 回答"这个模块有什么坑"，Basic Memory 回答"上次做到哪了"
...
**详细规范**：见 `.claude/commands/diagnose.md`
